  ...
  module Strategies
    class LDAP
      ...
      def callback_phase
        ...
        begin
          @ldap_user_info = @adaptor.bind_as(:filter => Net::LDAP::Filter.eq(@adaptor.uid, @options[:name_proc].call(request['username'] && request['username'].strip)),:size => 1, :password => request['password'])
